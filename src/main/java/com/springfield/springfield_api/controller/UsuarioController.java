...
public class UsuarioController {
    ...
    public ResponseEntity<?> realizarLogin(@RequestBody Map<String, String> credenciais) {
        try {
            ...
            if (usuario.getUltimoLogin() != null
                    && usuario.getUltimoLogin().isBefore(LocalDateTime.now().minusDays(30))) {
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                        .body(Map.of("erro", "Troca de senha obrigatória."));
            }
    ...
    public ResponseEntity<?> trocarSenha(@PathVariable Integer id, @RequestBody Map<String, String> request) {
        try {
